...
TamaGame.Player = function(game,id,infos) {
    ...
    Phaser.Sprite.call(this, game, posX + 50 , TamaGame.GAME_Y/2 + 100 , infos.img);
...
TamaGame.prototype = {
    ...
    preload : function() {
        for(var i = 0; i < TamaGame.MAX_PLAYERS ; i++) {
            game.load.image('road'+(i+1),TamaGame.IMG_FOLDER+'road'+(i+1)+'.jpg');
        }
        ...
        game.load.image('ange',TamaGame.IMG_FOLDER+'ange.png');
        game.load.image('neko',TamaGame.IMG_FOLDER+'neko.png');
    ...
    create : function() {
        ...
        { 			     	
            ...
            if (msgs[0] == "newPlayer") {
                ...
                //creer un joueur
                _this.createPlayer(parseInt(msgs[1]),msgs[3],msgs[2]);
            }
        ...
        for(var nb=1; nb <=4;nb++) {
            var posX = (TamaGame.GAME_X/TamaGame.MAX_PLAYERS)*(nb-1);
            var roadWidth = TamaGame.GAME_X/TamaGame.MAX_PLAYERS;
            var road = game.add.tileSprite(posX,0,roadWidth,TamaGame.GAME_Y,'road'+nb);        
            road.player = this;
            road.obstacles = [];
            this.roads.add(road);
        }
    ...
    createPlayer(indexPlayer,img,name) {
        ...
        //creation de la hitbox
        player.hitbox = game.add.sprite(player.x + 50, player.y, null);
        game.physics.enable(player.hitbox,Phaser.Physics.ARCADE);
        player.hitbox.enableBody = true;
        player.hitbox.body.immovable = true;
        player.hitbox.body.setSize(100,20);
        player.hitbox.player = player;
        this.hitboxes.add(player.hitbox);
    },
